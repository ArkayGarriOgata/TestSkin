...
function hideShowMenuIcon(){
	var xlWidth = $( document ).width();
	if(xlWidth < 1024){
		$("#menuicon").show();
	}else{
		$("#menuicon").hide();
...
function hideShowMenu(){
	var oLeft = $("#sidebar-nav").offset();
	//var oIconLeft = $("#menuicon").offset();
	...
	if(xlWidth < 1024){
		if(oLeft.left == 0){
			//oIconLeft.left = 5;
			//$("#menuicon").offset(oIconLeft);
			$("#sidebar-nav").removeClass("offcanvas-active");

		}else{
			//oIconLeft.left = 230;
			//$("#menuicon").offset(oIconLeft);
			$("#sidebar-nav").addClass("offcanvas-active");
		}
	}
}
...
function getVersion(){
	...
	var theRequest = new FourDWebRequest();
	theRequest.initWithAddress("", "","customer_Portal");
	theRequest.addParameter('Action', "GetVersion");

	theRequest.sendRequest(true, function() {
										   var ttResponse = this.fourDResponse;
										   if(ttResponse.length >0){
												$("#version").html("<em>Version "+ttResponse+"</em>");
	...
	theRequest.sendRequest(true, function() {
										...
										if(ttResponse.length >0){
											...
											if (obReturn.success == true){
												...
												window.location.href = 'menu.html?mode=navProductCode';
