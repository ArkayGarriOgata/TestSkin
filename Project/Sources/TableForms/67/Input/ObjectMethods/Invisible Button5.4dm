If (JML_CanThisJobBeMarkedComplete)
	If ([Job_Forms_Master_Schedule:67]DateComplete:15#!00-00-00!)
		Cal_getDate(->[Job_Forms_Master_Schedule:67]DateComplete:15; Month of:C24([Job_Forms_Master_Schedule:67]DateComplete:15); Year of:C25([Job_Forms_Master_Schedule:67]DateComplete:15))
	Else 
		Cal_getDate(->[Job_Forms_Master_Schedule:67]DateComplete:15)
	End if 
	...
	C_LONGINT:C283($err)
	$err:=sDateLimitor(->[Job_Forms_Master_Schedule:67]DateComplete:15; 7; 7)
...
Else 
	...
	[Job_Forms_Master_Schedule:67]DateComplete:15:=Old:C35([Job_Forms_Master_Schedule:67]DateComplete:15)
End if 