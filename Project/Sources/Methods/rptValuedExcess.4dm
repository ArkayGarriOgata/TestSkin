...
//Procedure: rptValuedExcess()  121197  MLB UPR1906
//based on rptNewInventor2, this report starts the same,
//but then for items which show excess, check the 
//release sched to see if there are forecasted 
//releases which cover some of the excess
//See also rptNewInventory,gPrintXMonthly•
// (via doFGRptRecords) 
...
//081000 DON'T accumulate negative excesses
...
If ($continue)
	...
	uINVauxil(7; ""; $numFGs)  //load aPayU2 & aQty which indicate if it is in the rpt, & the total excess  
	...
	If ($numOLs>0)
		...
		For ($i; 1; $numOLs)
			...
			If ($hit#-1)
				If (Not:C34(aPayU2{$hit}))  //not already in the report
					If (aQty{$hit}>0)
						$aAccumEx{$i}:=aQty{$hit}  //accumulate the excess
		...
		C_LONGINT:C283($col12; $col13; $col14; $col15)
		C_LONGINT:C283($lineCounter; $QtyBookTot; $QtyOrdTot; $QtyShpTot; $QtyOpenTot; $QtyExcTot)  //Customer totals
		C_LONGINT:C283($QtyBookSub; $QtyOrdSub; $QtyShpSub; $QtyOpenSub)  //brand subtotals
		C_LONGINT:C283($QtyBookGrnd; $QtyOrdGrnd; $QtyShpGrnd; $QtyOpenGrnd)  //rpt grand totals 
		...
		$header:=Change string:C234($header; (Uppercase:C13(t2)+", W/FORECAST"); 1)
		...
		For ($i; 1; $numOLs)
			...
			While ($aCustId{$j}=$currCust)  //process all of this customers ordelrines
				...
				//$formatLine:=Change string($formatLine;Substring($aDesc{$j};1;14);$col5)`•072696
				
				$formatLine:=Change string:C234($formatLine; String:C10($aQty{$j}; "^^,^^^,^^0"); $col5)
				//$QtyBookTot:=$QtyBookTot+$aQty{$j}
					...
					For ($k; $fgCursor; $numFGs)
						If (Substring:C12(<>aFGKey{$k}; 1; 5)=$currCust)  //init the array  to true
							If (Not:C34(aPayU2{$k}))  //put it on the report
								...
								//$formatLine:=Change string($formatLine;Substring($aBrand{$j};1;10);$col4)
								
								//$formatLine:=Change string($formatLine;Substring($aDesc{$j};1;14);$col5)        
		...
		//*    Check for inventory on inactive customers 
		If (tCust="")
			If (rReal1=0)  //zero month and up , this algo only works if all the cust orders were included   
				GOTO XY:C161(10; 15)
