...
If (Size of array:C274($cust_ids)>0)
	...
	If ($docRef#?00:00:00?)
		...
		If (Records in selection:C76([Finished_Goods:26])>0)
			xText:="DELETE FROM `item_masters` WHERE 1;"+Char:C90(13)
			xText:=xText+"DELETE FROM `jobs` WHERE 1;"+Char:C90(13)
			xText:=xText+"DELETE FROM `orders` WHERE 1;"+Char:C90(13)
			xText:=xText+"DELETE FROM `releases` WHERE 1;"+Char:C90(13)
		...
		If (Records in selection:C76([Finished_Goods:26])>0)
			If (Not:C34(<>modification4D_14_01_19))  // BEGIN 4D Professional Services : January 2019 Next record
				...
				While (Not:C34(End selection:C36([Finished_Goods:26])))
					...
					qty_onhand:=FG_Inventory_Array("lookupFG"; [Finished_Goods:26]FG_KEY:47)
			...
			Else 
				...
				SELECTION TO ARRAY:C260([Finished_Goods:26]ProductCode:1; $_ProductCode; [Finished_Goods:26]OutLine_Num:4; $_OutLine_Num; [Finished_Goods:26]CustID:2; $_CustID; [Finished_Goods:26]FG_KEY:47; $_FG_KEY; [Finished_Goods:26]OrderType:59; $_OrderType; [Finished_Goods:26]CartonDesc:3; $_CartonDesc; [Finished_Goods:26]Line_Brand:15; $_Line_Brand)
				
				SORT ARRAY:C229($_ProductCode; $_OutLine_Num; $_CustID; $_FG_KEY; $_OrderType; $_CartonDesc; $_Line_Brand; >)
				...
				For ($Iter; 1; Size of array:C274($_ProductCode); 1)
					...
					packing_spec:=String:C10($cases)+"x"+String:C10($packed_at)+"="+String:C10($cases*$packed_at)
					qty_onhand:=FG_Inventory_Array("lookupFG"; $_FG_KEY{$Iter})
					...
					$fg_desc:=Replace string:C233($fg_desc; "\\"; "-")
		...
		//**************************
		//send the jobits
		xText:="INSERT INTO `jobs` (`access_filter`,  `product_code`,  `date_planned`, "
		xText:=xText+"`qty_want`, `qty_actual`, `ams_ref`) VALUES "+Char:C90(13)
		...
		If (Records in selection:C76([Customers_Order_Lines:41])>0)
			...
			Else 
				...
				SELECTION TO ARRAY:C260([Customers_Order_Lines:41]ProductCode:5; $_ProductCode; [Customers_Order_Lines:41]CustID:4; $_CustID; [Customers_Order_Lines:41]CustomerLine:42; $_CustomerLine; [Customers_Order_Lines:41]PONumber:21; $_PONumber; [Customers_Order_Lines:41]defaultBillto:23; $_defaultBillto; [Customers_Order_Lines:41]Quantity:6; $_Quantity; [Customers_Order_Lines:41]DateOpened:13; $_DateOpened; [Customers_Order_Lines:41]OrderLine:3; $_OrderLine; [Customers_Order_Lines:41]Qty_Open:11; $_Qty_Open)
				
				SORT ARRAY:C229($_ProductCode; $_CustID; $_CustomerLine; $_PONumber; $_defaultBillto; $_Quantity; $_DateOpened; $_OrderLine; $_Qty_Open)
		...
		If (Records in selection:C76([Customers_ReleaseSchedules:46])>0)
			...
			Else 
				...
				SELECTION TO ARRAY:C260([Customers_ReleaseSchedules:46]ProductCode:11; $_ProductCode; [Customers_ReleaseSchedules:46]OrderLine:4; $_OrderLine; [Customers_ReleaseSchedules:46]CustID:12; $_CustID; [Customers_ReleaseSchedules:46]CustomerLine:28; $_CustomerLine; [Customers_ReleaseSchedules:46]CustomerRefer:3; $_CustomerRefer; [Customers_ReleaseSchedules:46]Shipto:10; $_Shipto; [Customers_ReleaseSchedules:46]Sched_Qty:6; $_Sched_Qty; [Customers_ReleaseSchedules:46]Sched_Date:5; $_Sched_Date; [Customers_ReleaseSchedules:46]Promise_Date:32; $_Promise_Date; [Customers_ReleaseSchedules:46]ReleaseNumber:1; $_ReleaseNumber; [Customers_Order_Lines:41]Status:9; $_Status)
				
				SET FIELD RELATION:C919([Customers_ReleaseSchedules:46]OrderLine:4; $lienAller; $lienRetour)
				SORT ARRAY:C229($_ProductCode; $_OrderLine; $_CustID; $_CustomerLine; $_CustomerRefer; $_Shipto; $_Sched_Qty; $_Sched_Date; $_Promise_Date; $_ReleaseNumber; $_Status)
		...
		//**************************
		//determine the conduit
		//**************************
		
		Case of 
			: (Count parameters:C259=0)  //mysql insert
				// Modified by: Mel Bohince (7/22/16) try more than once if failed connection
				$number_of_tries:=0
				Repeat 
					DELAY PROCESS:C323(Current process:C322; (60*5))
					$number_of_tries:=$number_of_tries+1
					conn_id:=CUSTPORT_ConnectionManager("Open")
				Until (conn_id#0) | ($number_of_tries=3)
				
				If (conn_id#0)
					DOCUMENT TO BLOB:C525(docName; $sql_blob)
					//$affected_rows:=MySQL Execute Blob (conn_id;$sql_blob)
					conn_id:=CUSTPORT_ConnectionManager("Close")
					SET BLOB SIZE:C606($sql_blob; 0)
				Else 
					EMAIL_Sender("CUSTPORT_Export"; ""; "connection to dreamhost failed"; ""; ""; ""; "mel.bohince@arkay.com")
				End if 
				
			: (Count parameters:C259=1)  //by email address
				xTitle:="Customer Protal Data "+String:C10(Current date:C33; Internal date short:K1:7)
				xText:="As of "+String:C10(Current date:C33; Internal date short:K1:7)+$r+"Import the attached with phpMyAdmin"
				EMAIL_Sender(xTitle; ""; xText; $1; docName)
				
			: (Count parameters:C259>1)  //by ftp to server address
				$shortname:=HFSShortName(docName)
				$where:="/arkay.info/uploads/"+$shortname
				$ftp_ID:=0
				zwStatusMsg("CUST_PORTAL"; "FTP to "+$1+$where)
				Case of 
					: (FTP_Login($1; $2; $3; $ftp_ID)#0)  //"bohince.com";"admin_bohince";"8CeUJx3z"
					: (FTP_SetPassive($ftp_ID; 1)#0)
					: (FTP_Send($ftp_ID; docName; $where; 0)#0)
					: (FTP_Logout($ftp_ID)#0)
					Else 
						$ftp_ID:=0  //all commands executed without error
				End case 
		End case 
