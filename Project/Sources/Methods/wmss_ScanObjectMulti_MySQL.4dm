...
//Method: WMS_ScanObjectMulti_MySQL - Created v0.1.0-JJG (05/06/16)
If (<>fDebug)
	DODEBUG(Current method name:C684)
End if 

// ----------------------------------------------------
// Method: wmss_ScanObjectMulti   ( ) ->
// By: Mel Bohince @ 04/06/16, 16:27:21
// Description
// based on wmss_ScanObject
// ----------------------------------------------------
	...
	If ($conn_id>0)
		...
		If ($row_count>0)  //display what is going to move
			...
			If (rft_object="skid")
				...
				APPEND TO ARRAY:C911(rft_Skid; $rft_Skid{1})
				APPEND TO ARRAY:C911(rft_Bin; $rft_Bin{1})
				APPEND TO ARRAY:C911(rft_jobit; $rft_jobit{1})
				APPEND TO ARRAY:C911(rft_Status; wmss_CaseState($acase_state{1}))
				$skidQty:=0
				For ($i; 1; $row_count)
					$skidQty:=$skidQty+Num:C11(WMS_CaseId($rft_Case{$i}; "qty"))
				End for 
				iQty:=iQty+$skidQty
				$strQty:=txt_Pad(String:C10($skidQty); " "; 3; 14)
				
				APPEND TO ARRAY:C911(rft_Case; $strQty+String:C10($row_count)+" cases")
			...
			Else   //scanned a case
				...
				If ($continue)
					For ($i; 1; $row_count)
						...
						APPEND TO ARRAY:C911(ListBox1; False:C215)
						APPEND TO ARRAY:C911(rft_Case; $rft_Case{$i})
						APPEND TO ARRAY:C911(rft_Skid; $rft_Skid{$i})
						APPEND TO ARRAY:C911(rft_Bin; $rft_Bin{$i})
						APPEND TO ARRAY:C911(rft_jobit; $rft_jobit{$i})
						APPEND TO ARRAY:C911(rft_Status; $rft_Status{$i})
					End for 
				End if   //continuew
