...
//PM:  Invoice_NewBillandHold  072299  mlb
//instantiate a new invoice, 
//return invoice number if success, 0 or negative if fail
//called by uFGBillnHold, replaces api_SendInvTran
// Modified by: Mel Bohince (10/8/19) include the shipto on the invoice so YTD reprot can show the destination
...
If (Length:C16($exception)=0)
	...
	[Customers_Invoices:88]ShipTo:9:=[Customers_Order_Lines:41]defaultShipTo:17  // Modified by: Mel Bohince (10/8/19) include the shipto on the invoice so YTD reprot can show the destination
