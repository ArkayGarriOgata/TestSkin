...
// ----------------------------------------------------
// User name (OS): mel
// Date and time: 03/12/09, 10:41:33
// ----------------------------------------------------
// Method: rptAgeFGfifo_excess
// Description
// strip valued and value from rptAgeFGfifo_
...
xTitle:="Aged F/G EXCESS Inventory, by Date Glued, using FIFO Valuation "+String:C10($today; <>LONGDATE)+"  "+String:C10(4d_Current_time; <>HMMAM)+" Open Orders includes over-run, Open releases includes forecasts"
xText:=""+$t+""+$t+""+$t+""+$t+$t+""+$t+""+$t+"≤ 3 Mths"+$t+"3 < Mths ≤ 6"+$t+"6 < Mths ≤ 9"+$t+"9 < Mths ≤ 12"+$t+"12 < Mths"+$t+"Glued=00/00/00"+$cr+$cr
xText:=xText+"SALESMAN"+$t+"CUSTOMER"+$t+"CPN"+$t+"NO_COST"+$t+"BILL&HOLD"+$t+"OPEN_PO"+$t+"RELEASED"+$t+"Qty3"+$t+"Qty6"+$t+"Qty9"+$t+"Qty12"+$t+"Qty13"+$t+"Qty?"+$t+"LastGlued"+$t+"Line"+$t+"Desc"+$cr

docName:="AgeFGinvenFIFO_Excess"+fYYMMDD($today)+"_"+Replace string:C233(String:C10(4d_Current_time; <>HHMM); ":"; "")
...
For ($i; 1; Size of array:C274(<>aFGKey))
	...
	For ($grp; 6; 1; -1)  //set qty upto any excess, from oldest to newest
		If ($qtyGroup{$grp}>0)  //then it may be excess
			Case of 
				: ($excess=0)  //no excess, so show nothing
					$qtyGroup{$grp}:=0
				: ($excess>=$qtyGroup{$grp})  //lots of excess, so all this qty is excess
					$qtyGroup{$grp}:=$qtyGroup{$grp}  //all that qty is excess
					$excess:=$excess-$qtyGroup{$grp}
				Else   //some of the qty is excess
					$qtyGroup{$grp}:=$excess  //remove the valued part
					$excess:=0
			End case 
		End if 
	End for 
	...
	If (($qtyGroup{1}+$qtyGroup{2}+$qtyGroup{3}+$qtyGroup{4}+$qtyGroup{5}+$qtyGroup{6})>0)
		xText:=xText+$aSalesman{$i}+$t+$aCustName{$i}+$t+Substring:C12(<>aFGKey{$i}; 7)+$t+String:C10($ttl_excess)+$t+String:C10($aBillHold{$i})+$t+String:C10($orders)+$t+String:C10($releases)+$t
		For ($grp; 1; 6)
			xText:=xText+String:C10($qtyGroup{$grp})+$t
		End for 
