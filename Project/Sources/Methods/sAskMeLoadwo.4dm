...
//Procedure: sAskMeLoadwo(sCPN)  020896  MLB
//load the quadrants without including customer search
//see also sAskMeLoadwith
//• 1/9/98 cs Lena wants to see B&H too
//•5/04/99  MLB  don't show rejected orderlines
...
Else 
	...
	SET QUERY DESTINATION:C396(Into set:K19:2; "threeLoaded")
	QUERY:C277([Finished_Goods_Locations:35]; [Finished_Goods_Locations:35]ProductCode:1=$sCPN)  //switch to fg_key
	...
	If (allinventory=0)
		...
		QUERY BY FORMULA:C48([Finished_Goods_Locations:35]; \
			([Finished_Goods_Locations:35]ProductCode:1=$sCPN)\
			 & (([Finished_Goods_Locations:35]Location:2="FG:@") | ([Finished_Goods_Locations:35]Location:2="BH:@"))\
			)
...
// SEARCH SELECTION BY FORMULA([FG_Locations];[FG_Locations]Location="FG:@")
...
If (allreleases=0)
	If (Not:C34(<>modification4D_28_02_19))  // BEGIN 4D Professional Services : January 2019 next record
		...
		//SEARCH SELECTION BY FORMULA([OrderLines];[OrderLines]Status="Closed")
...
Else 
	...
	SET QUERY DESTINATION:C396(Into set:K19:2; "fourLoaded")
	QUERY:C277([Job_Forms_Items:44]; [Job_Forms_Items:44]ProductCode:3=$sCPN)  //switch to fg_key
	...
	If (allJobs=0)
		...
		QUERY BY FORMULA:C48([Job_Forms_Items:44]; \
			([Job_Forms_Items:44]ProductCode:3=$sCPN)\
