//%attributes = {}
// -------
// Method: Job_WIP_Kanban_Skid_Count   ( {"email"} ) ->
// By: Mel Bohince @ 01/29/19, 13:23:17
// Description
// based on Job_WIP_Kanban changing sheet count to approximate skid counts
// and leaving out the scheduling and thruput crap
// assuming 40 inch tall loads and sheets per load is inches divided by caliper
// ----------------------------------------------------
...
SELECTION TO ARRAY:C260([Job_Forms:42]JobFormID:5; $aJobforms; [Job_Forms:42]PlnnerReleased:59; $aPlnReleased; [Job_Forms:42]EstGrossSheets:27; $aGrossSheets; [Job_Forms:42]QtyYield:30; $aCartonQty; [Job_Forms:42]cust_id:82; $aJobCust; [Job_Forms:42]CustomerLine:62; $aJobLine; [Job_Forms:42]JobType:33; $aJobType; [Job_Forms:42]Caliper:49; $aJobCaliper)  //[Job_Forms]ProducedQty
...
For ($job; 1; $numElements)
	...
	//If (Position("12920.28";$aJobforms{$job})>0)
	//TRACE
	//End if 
	$wip:=0
			...
			For ($seq; 1; Size of array:C274($mtJseq))
				...
				Else   //walk the walk,
					...
					If ($hit>-1)  //whew, this is normal, 
						...
						$preConvertedSheets:=$sheets
						$sheets:=Job_ConvertSheetsToLoads($sheets; $aJobCaliper{$job})
						...
						Case of   //see uInit_CostCenterGroups
							...
							: (Position:C15($nextCC; <>GLUERS)>0)
								...
								APPEND TO ARRAY:C911($aIsGlueing; ($aCartonQty{$job}-$preConvertedSheets))  //in this case sheets are really cartons, so find how many more to glue
		...
		Else   //see if board or plastic was issued
			...
			If (Records in selection:C76([Raw_Materials_Transactions:23])>0)
				...
				If ([Raw_Materials:21]IssueUOM:10="SHT")
					...
					$quePrinting:=Job_ConvertSheetsToLoads($quePrinting; $aJobCaliper{$job})
					APPEND TO ARRAY:C911($aJobPrinting; $aJobforms{$job})
				...
				Else 
					...
					$queSheeter:=Job_ConvertSheetsToLoads($queSheeter; $aJobCaliper{$job})
					APPEND TO ARRAY:C911($aJobSheeter; $aJobforms{$job})
	...
	End if   //$nextSeq
	$aJobData{$job}:=$aJobforms{$job}+$delim+$mnemonic+$delim+CUST_getName($aJobCust{$job}; "el")+$delim+$aJobLine{$job}  //+$delim+String([Job_Forms_Master_Schedule]FirstReleaseDat;Internal date short special)+$delim+String([Job_Forms_Master_Schedule]ThroughPut)+$delim+String($wip)+$delim+String([Job_Forms_Master_Schedule]DateStockRecd;Internal date short special)+$delim+String($jobLastTouch;Internal date short special)  //+String($queSheeter)+$delim+String($quePrinting)+$delim+String($queStamping)+$delim+String($queEmbossing)+$delim+String($queLaminating)+$delim+String($queOther)+$delim+String($queBlanking)+$delim+String($queGlueing)+$delim+String($isGlueing)+"\r"
...
For ($queue; 1; Size of array:C274($aJobSheeter))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+String:C10($aQueSheeter{$queue})+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+"\r"
...
For ($queue; 1; Size of array:C274($aJobPrinting))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+String:C10($aQuePrinting{$queue})+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobStamping))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+String:C10($aQueStamping{$queue})+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobEmbossing))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+$pH+$delim+String:C10($aQueEmbossing{$queue})+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobLaminating))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+String:C10($aQueLaminating{$queue})+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobOther))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+String:C10($aQueOther{$queue})+$delim+$pH+$delim+$pH+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobBlanking))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+String:C10($aQueBlanking{$queue})+$delim+$pH+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobGlueing))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+String:C10($aQueGlueing{$queue})+$delim+$pH+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
For ($queue; 1; Size of array:C274($aJobIsGlueing))
	...
	If ($hit>-1)
		$outputLine:=$aJobData{$hit}+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+$pH+$delim+String:C10($aIsGlueing{$queue})+"\r"
		APPEND TO ARRAY:C911($aOutput; $outputLine)
	Else 
		ALERT:C41("error")
	End if 
...
$t:=$delim
$r:="\r"
$title:=(6*$t)+"PALLETS IN WIP BY DEPARTMENT SORTED DESCENDING BY THRU-PUT DOLLARS (8WKS FIRM&FCST)"
$text:=""
...
If ($docRef#?00:00:00?)
	...
	$text:=$text+"JOB_FORM"+$t+"ROUTING"+$t+"CUSTOMER"+$t+"LINE"+$t+"SHEETING"+$t+"PRINTING"+$t+"STAMPING"+$t+"EMBOSSING"+$t+"LAMINATING"+$t+"OTHER"+$t+"BLANKING"+$t+"TO_GLUE(ctns)"+$t+"GLUEING(ctns)"+$r
	...
	If (Count parameters:C259=1)
		...
		EMAIL_Sender("Kanban Pallet Status"; ""; $body; "mel.bohince@arkay.com"; $docName)
