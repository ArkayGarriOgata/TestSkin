...
// Modified by: Mel Bohince (2/26/18) see orig at PF_JobformToXML3level
// Modified by: Mel Bohince (10/2/18) change EarliestDate
...
If (False:C215)  // Modified by: Mel Bohince (7/9/21) disable
	...
	//READ ONLY([Job_Forms_Master_Schedule])
	...
	If (Records in selection:C76([Job_Forms:42])=1)
		...
		$title:=""
		$text:=""
		$docName:="Job_"+Replace string:C233($jobform; "."; "_")+"_"+fYYMMDD(4D_Current_date)+"_"+Replace string:C233(String:C10(4d_Current_time; HH MM SS:K7:1); ":"; "")+".xml"
		...
		If (True:C214)  //jobs-job
			...
			If ([Job_Forms_Master_Schedule:67]MAD:21#!00-00-00!)  // Modified by: Mel Bohince (3/1/18) 
				APPEND TO ARRAY:C911($AttrVal; PF_util_FormatDate(->[Job_Forms_Master_Schedule:67]MAD:21))
			Else 
				APPEND TO ARRAY:C911($AttrVal; PF_util_FormatDate(->[Job_Forms_Master_Schedule:67]TargetDate_PrintFlow:88))
			End if 
			
			// Modified by: Mel Bohince (10/2/18) 
			APPEND TO ARRAY:C911($AttrName; "EarliestDate")  //5
			$epd:=[Job_Forms_Master_Schedule:67]EarliestStart_PrintFlow:90  //Add to date(current date;0;0;7)  //arbitrary after current date
			...
			APPEND TO ARRAY:C911($AttrVal; [Job_Forms:42]JobType:33)
			...
			APPEND TO ARRAY:C911($AttrVal; Substring:C12(CUST_getName([Jobs:15]CustID:2; "elc"); 1; 20))
		...
		If ($sendProperties)  //form prop
			...
			//PROPERTIES:
			PF_AddProperty($propref; "Die Number"; [Job_Forms:42]OutlineNumber:65)
			...
			$rmWidth:=String:C10(Job_getBoardWidth($jobform))  //based on rm item mstr
			...
			//PF_AddProperty ($propref;"sheet-wrap";Job_is_Wrap ($jobform))  //Raw_Materials where CommodityCode = 1 and Flex1 >= 80 and Flex1 <= 100
			//RESOURCES:
			//none
		End if   //form prop
