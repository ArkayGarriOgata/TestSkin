...
// Modified by: Mel Bohince (9/10/21) use [Customers]Std_Terms instead of the PAT segment
...
C_TEXT:C284($r)
C_REAL:C285($price)
C_BOOLEAN:C305($inHeader; changeOrderOpened)  //keep track of the level on indent
sCPN:=""
...
For ($segment; 1; $numberOfSegments)
	...
	Case of 
		...
		: ($tag="BGM")
			...
			If (Records in selection:C76([Customers_Orders:40])=0)
				QUERY:C277([Customers_Orders:40]; [Customers_Orders:40]PONumber:11="BR0"+$_PO)  //old edirk style po
			End if 
			...
			If (Records in selection:C76([Customers_Orders:40])=0)
				...
				[Customers_Orders:40]Terms:23:="Net 60"  //a default// Modified by: Mel Bohince (9/10/21) 
			Else 
		...
		: ($tag="UNS")
			Case of 
				...
				: (util_TextParser(1)="S")
					$inHeader:=True:C214
					If (False:C215)  //apply status changes on existing order after we know the response
						//If ([Customers_Orders]edi_status="CHANGE")  `price or quantity changed, then re-book by setting to CONTRACT
						//If (Not($new_order_line))  `otherwise it has already been set to contract
						//If ([Customers_Order_Lines]Status#"closed")  `don't mess with closed orderline statii
						//If (($changed_price) | ($changed_quantity))  `don't rebook if only a date or destination change
						//[Customers_Order_Lines]Status:="CONTRACT"
						//If ([Customers_Orders]Status="Accepted") | ([Customers_Orders]Status="")
						//[Customers_Orders]Status:="CONTRACT"
						//End if 
						//End if 
						//End if 
						//End if 
					...
					If ($ok_to_save)
						...
						//SAVE RECORD([Customers_Order_Change_Orders])
						//End if 
					End if 
			End case 
		...
		: ($tag="LIN")
			...
			Case of 
				...
				: (Senders_ID="6314547000Q")  // Modified by: Mel Bohince (4/27/20) 
					$_PO_Item:=String:C10($line_item; "00000")
					$line_item:=$line_item/10
				Else 
					$_PO_Item:=String:C10($line_item; "00000")
			...
			If (Length:C16(sCPN)>0)  //EL User error: The event occurs when the planner adds TEXT at the header level and does not change anything in the detail.
				...
				If (Records in selection:C76([Customers_Order_Lines:41])=0)  // Modified by: mel (12/10/09)
				...
				If (Length:C16([Customers_Order_Lines:41]ProductCode:5)>0)
					//cco_set_change_order_type (->sCPN;->[Customers_Order_Lines]ProductCode;->[Customers_Order_Change_Orders]CPNchg)
				End if 
				...
				If ($num_FG>0)  //see also button on fg.input "Update EDI ORders" **********
					...
					//Modified by: Mel Bohince (9/10/21) use the aMs customer's record terms
				...
				//QUERY([Customers_ReleaseSchedules]; & ;[Customers_ReleaseSchedules]OpenQty>0)` Modified by: mel (12/7/09)
				If (Records in selection:C76([Customers_ReleaseSchedules:46])=0)  // Modified by: mel (12/10/09)
					QUERY:C277([Customers_ReleaseSchedules:46]; [Customers_ReleaseSchedules:46]CustomerRefer:3=([Customers_Order_Lines:41]PONumber:21+".001"))
				End if 
				If (Records in selection:C76([Customers_ReleaseSchedules:46])=0)  // & (Length([Customers_Order_Lines]ProductCode)>0)
		...
		: ($tag="RFF") & (Length:C16(sCPN)>0)
			...
			[Customers_ReleaseSchedules:46]CustomerRefer:3:=[Customers_Order_Lines:41]PONumber:21+"."+$_Release
			
		: ($tag="CUX")
			$_order_comments:=$_order_comments+Substring:C12(util_TextParser(1); 1; 3)+Char:C90(13)
			
		: ($tag="PAT")
			If (False:C215)  // Modified by: Mel Bohince (9/10/21) see above based on customer record terms field
				If (Senders_ID#"BEESL.ESL001")
