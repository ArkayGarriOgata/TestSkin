...
//Method: wms_api_get_recon_racks_only - Created v0.1.0-JJG (05/18/16) -re-written for MySQL vs 4D
If (<>fDebug)
...
If (<>fWMS_Use4D)
	WMS_API_4D_getReconRacksOnly
Else 
	wms_api_MySQL_getReconRacksOnly
End if 


If (False:C215)  //v0.1.0-JJG (05/18/16) -moved to wms_api_MySQL_getReconRacksOnly
	//  // ----------------------------------------------------
	//  // Method: wms_api_get_recon_racks_only   ( ) ->
	//  // By: Mel Bohince @ 03/22/16, 16:29:37
	//  // Description
	//  // based on wms_api_get_reconciliation
	//  // ----------------------------------------------------
	...
	//$matchString1:="BNV-"
	//$matchString2:="BNR-"
	//$matchString:=$matchString1+" "+$matchString2
	...
	//QUERY([Finished_Goods_Locations]; & ;[Finished_Goods_Locations]Tier>0)
	...
	//If (Position($binPrefix;$matchString)>0)  //If ($aCases_Location{$location}="BNV-@")// |($aCases_Location{$location}="BNR-@") //rack location
