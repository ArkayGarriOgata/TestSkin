...
CONFIRM:C162("Hide Obsoleted Finished Goods?"; "Hide"; "Show")

If (OK=1)
	$hideObsolete:=True:C214
Else 
	$hideObsolete:=False:C215
End if 
CONFIRM:C162("Hide Closed Orderlines?"; "Hide"; "Show")
If (OK=1)
	$hideClosed:=True:C214
Else 
	$hideClosed:=False:C215
End if 
//*   read only files
...
zwStatusMsg("MaryKay"; " Searching Mary Kay Cosmetics...")
$CustName:="Mary Kay Cosmetics"
...
bCancel:=0
$Continue:=True:C214
CONFIRM:C162("Do you Want to print this report to a Disk File?"; "Disk"; "Paper")
If (ok=1)  //*   maybe create path to folder
	$Print2Disk:=True:C214
	$Path:=uCreateFolder("MaryKayReports_"+fYYMMDD(4D_Current_date))  //•091098  MLB  
Else 
...
If ($Continue)  //•091098  MLB  give earlyer warning
	...
	If (Not:C34(<>modification4D_14_01_19))  // BEGIN 4D Professional Services : January 2019 query selection
		...
		If ($hideClosed)  //• mlb - 3/20/03  15:23
	...
	Else 
		...
		// i used the all on the line 121
		QUERY:C277([Customers_Order_Lines:41]; [Customers_Order_Lines:41]CustID:4=$CustId; *)
		QUERY:C277([Customers_Order_Lines:41];  & ; [Customers_Order_Lines:41]Status:9#"Cancel@"; *)  //•080195  MLB 1490
		QUERY:C277([Customers_Order_Lines:41];  & ; [Customers_Order_Lines:41]Status:9#"Kill"; *)  //•080195  MLB 1490   
		If ($hideClosed)  //• mlb - 3/20/03  15:23
	...
	If (Records in selection:C76([Customers_Order_Lines:41])=0)
		...
		Else 
			...
			//we have forget this on previous work
		...
		Else 
			If ($hideObsolete)
				If (Not:C34(<>modification4D_14_01_19))  // BEGIN 4D Professional Services : January 2019 query selection
					...
					util_outerJoin(->[Finished_Goods:26]ProductCode:1; ->[Customers_Order_Lines:41]ProductCode:5)
					QUERY SELECTION:C341([Finished_Goods:26]; [Finished_Goods:26]Status:14="@obsolete@")
					util_outerJoin(->[Customers_Order_Lines:41]ProductCode:5; ->[Finished_Goods:26]ProductCode:1)
					
					CREATE SET:C116([Customers_Order_Lines:41]; "obseleties")
					DIFFERENCE:C122("All"; "obseleties"; "nonObsolete")
					USE SET:C118("nonObsolete")
					CLEAR SET:C117("nonObsolete")
					CLEAR SET:C117("obseleties")
					
				Else 
					
					zwStatusMsg("Relating"; " Searching "+Table name:C256(->[Customers_Order_Lines:41])+" file. Please Wait...")
					...
					End if 
					QUERY:C277([Customers_Order_Lines:41]; [Customers_Order_Lines:41]SpecialBilling:37=False:C215; *)
					QUERY:C277([Customers_Order_Lines:41]; #; [Finished_Goods:26]Status:14="@obsolete@")
	...
	Else   //*    Bail out if any found
		$Continue:=False:C215
		ALERT:C41("One or more Orderlines for Mary Kay have no 'Bill to'"+Char:C90(13)+"Please correct this before running this report."+Char:C90(13)+"See report currently printing to determine which orderlines.")
		xText:=""
		xTitle:="Mary Kay Orderlines without 'Bill Tos'"
		If (Not:C34(<>modification4D_14_01_19))  // BEGIN 4D Professional Services : January 2019 query selection
		...
		Else 
			...
			For ($Iter; 1; Size of array:C274($_OrderLine); 1)
				xText:=xText+$_OrderLine{Iter}+Char:C90(13)
			End for 
			
		End if   // END 4D Professional Services : January 2019 query selection
		
		rPrintText("No Bill Tos")
	End if 
	If (Not:C34(<>modification4D_14_01_19))  // BEGIN 4D Professional Services : January 2019 query selection
...
If ($Continue)
	...
	$Continue:=MKHeaderPrint3($Print2Disk; $CurrentBill; $Path)  //this places the correct header on the first report
	If ($Continue)
		xText:=""
		...
		For ($i; 1; $numOL)
			...
			Else 
				...
				If ($CurrentBill#$aOrdBillto{$i})  //• 5/18/98 cs this item is a change of shipping company            
					...
					MKHeaderPrint3($Print2Disk; $CurrentBill; $Path)  //start new report for different shipping address
					xText:=""
				End if 
