...
//Method: wms_api_MySQL_getReconRacksOnly - Created v0.1.0-JJG (05/18/16) - moved here from wms_api_get_recon_racks_only
If (<>fDebug)
	...
	If ($number_of_jobits>0)
		...
		For ($wms_jobit; 1; $number_of_jobits)
			...
			For ($location; 1; $number_of_locations)
				$binPrefix:=Substring:C12($aCases_Location{$location}; 1; 4)
				If (Position:C15($binPrefix; $matchString)>0)  //If ($aCases_Location{$location}="BNV-@")// |($aCases_Location{$location}="BNR-@") //rack location
