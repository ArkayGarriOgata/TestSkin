...
// Modified by: Mel Bohince (01/05/22)include rm with inventory without po's or allocations
// Modified by: MelvinBohince (4/29/22) when converting uom of allocation, don't forget to reduce by qty issued

C_TEXT:C284($commodity; $1; $0)

$commodity:=$1  //this should be like 01@ for all or 01-SBS@ for a subgroup 

C_BOOLEAN:C305($convert; $showAlloctions; $showPOs)
$convert:=(Num:C11($commodity)=1)  //change LF to Sheets
$showAlloctions:=True:C214  //legacy had option
$showPOs:=True:C214  //legacy had option
...
// Modified by: Mel Bohince (01/05/22)include rm with inventory without po's or allocations
If (Position:C15("01"; $commodity)>0)  // Modified by: Mel Bohince (11/16/21) include plastic with board
	$rm_es:=ds:C1482.Raw_Materials.query("(Commodity_Key=:1 or Commodity_Key=:2) and ((ALLOCATIONS.JobForm#:3) or (PURCHASE_ITEMS.Qty_Open>:4) or (RM_LOCATIONS.QtyOH>:5))"; $commodity; "20@"; ""; 0; 0).orderBy("Raw_Matl_Code")
Else 
	$rm_es:=ds:C1482.Raw_Materials.query("Commodity_Key=:1 and ((ALLOCATIONS.JobForm#:2) or (PURCHASE_ITEMS.Qty_Open>:3)or (RM_LOCATIONS.QtyOH>:4))"; $commodity; ""; 0; 0).orderBy("Raw_Matl_Code")
End if 
...
For each ($rm_e; $rm_es; $loc_e)  //looping thru the raw material record with jobs or po's or inventory
	...
	If (True:C214)  //merge the job allocations with the purchase orders
		...
		For each ($change; $changes_c)
			If (OB Is defined:C1231($change; "JobForm"))  //then this is an allocation,  decrement
				
				If ($change.UOM#$rm_e.IssueUOM) & ($convert)  //the allocation's uom doesn't match the rm's uom, so convert $change.Qty_Allocated to sheets
					$openAllocation:=RM_LF_to_SHT($change.JobForm)-$change.Qty_Issued  // Modified by: MelvinBohince (4/29/22) when converting uom of allocation, don't forget to reduce by qty issued
					$change.UOM:="SHT*"
			...
			Else   //this is a po line, increment
