...
If (Position:C15("01"; $commodity)>0)  // Modified by: Mel Bohince (11/16/21) include plastic with board
	$rm_es:=ds:C1482.Raw_Materials.query("(Commodity_Key=:1 or Commodity_Key=:2) and ((ALLOCATIONS.JobForm#:3) or (PURCHASE_ITEMS.Qty_Open>:4))"; $commodity; "20@"; ""; 0).orderBy("Raw_Matl_Code")
Else 
...
If (False:C215)
	// this would match legacy allocation rpt showing all rm with allocation, po, or existing inventory
	//coupled with the If($rm_e.RM_LOCATIONS.length>0) skipping r/m's without inventory in the forEach below
	$rm_es:=ds:C1482.Raw_Materials.query("Commodity_Key=:1 or Commodity_Key=:2"; $commodity; "20@").orderBy("Raw_Matl_Code")
End if 
...
For each ($rm_e; $rm_es; $loc_e)  //looping thru the raw material record with jobs or po's
	...
	//If ($rm_e.RM_LOCATIONS.length>0)  //has inventory
