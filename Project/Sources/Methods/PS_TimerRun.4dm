...
//PM: JML_PressScheduleTimerRun() -> 
//@author mlb - 4/9/02  14:09
...
If (Records in set:C195("clickedIncludeRecord")=1)
	...
	If (Records in selection:C76([Job_Forms_Machines:43])>0)
		$mrHrs:=Time:C179(Time string:C180(([Job_Forms_Machines:43]Planned_RunHrs:37*3600)))
		REDUCE SELECTION:C351([Job_Forms_Machines:43]; 0)
		//$mrHrs:=Time(Time string((0.05*3600)))
		zwStatusMsg("RUN TIMER"; "  Run= "+String:C10($mrHrs; HH MM SS:K7:1)+" Timer started at "+String:C10(Current time:C178; HH MM SS:K7:1))
		$pid:=New process:C317("util_countDownTimer"; <>lMinMemPart; "$Timer"; [ProductionSchedules:110]CostCenter:1+": "+[ProductionSchedules:110]JobSequence:8+" Run Remaining"; Current time:C178+$mrHrs)
		If (False:C215)
			util_countDownTimer
		End if 
