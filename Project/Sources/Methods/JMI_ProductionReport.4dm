...
If (ok=1)
	...
	If (ok=1)
		...
		If ($docRef#?00:00:00?)
			...
			$text:=$text+"JOBIT"+$delim+"SKID"+$delim+"CASES"+$delim+"QUANTITY"+$delim+"WHEN_SCANNED"+$delim+"WHERE_NOW"+$delim+"WHEN_NEEDED"+$delim+"PRODUCT_CODE"+"\r"
			$ttlCases:=0
			$ttlQty:=0
			$numSkids:=0
			$ttlReceiptQty:=0
			For ($i; 1; $numSkidTransactions)
				...
				$numSkids:=$numSkids+1
				$text:=$text+$aJobit{$i}+$delim+"#"+$aSkid{$i}+$delim+String:C10($aNumCases{$i})+$delim+String:C10($aQty{$i})+$delim+$aTransAt{$i}+$delim+$aLocation{$i}+$delim+String:C10($aReleaseDate{$i}; Internal date short special:K1:4)+$delim+$aProductCode{$i}+"\r"
			End for 
			$text:=$text+"\rTOTALS:"+$delim+String:C10($numSkids)+$delim+String:C10($ttlCases)+$delim+String:C10($ttlQty)+$delim+""+$delim+""+"\r"
			
			$text:=$text+"\r\rRECEIPTS SUMMARY BY JOBIT:"+"\r"
			$text:=$text+"JOBIT"+$delim+"PRODUCT_CODE"+$delim+"CASES"+$delim+"QUANTITY"+$delim+"TTL_RECEIVED"+$delim+"SKIDS"+$delim+"MACH_TIC_QTY"+$delim+"TTL_MACH_TIC_GOOD"+$delim+"WHEN_NEEDED"+"\r"
			$ttlJobitCases:=0
			$ttlJobitSkids:=0
			$ttlJobitQty:=0
			$ttlReceiptQty:=0
			...
			For ($i; 1; $numJobits)
				...
				Else 
					$relDate:=<>MAGIC_DATE
				End if 
				$text:=$text+$aJobitSum{$i}+$delim+$aCPN{$i}+$delim+String:C10($aCaseCount{$i})+$delim+String:C10($aQtyTTl{$i})+$delim+String:C10($aTTLreceipts{$i})+$delim+String:C10($aSkidCount{$i})+$delim+String:C10($aMachTickQty{$i})+$delim+String:C10($aMachTickAllQty{$i})+$delim+String:C10($relDate; Internal date short special:K1:4)+"\r"
			End for 
			$text:=$text+"\rTOTALS"+$delim+""+$delim+String:C10($ttlJobitCases)+$delim+String:C10($ttlJobitQty)+$delim+String:C10($ttlReceiptQty)+$delim+String:C10($ttlJobitSkids)+$delim+String:C10($ttlJobitMachtic)+$delim+String:C10($ttlAllJobitMachtic)+"\r"
			...
			$text:=$text+"BOL_NUMBER"+$delim+"NUM_SKIDS"+"\r"
			$ttlJobitSkids:=0
			For ($i; 1; Size of array:C274($aBin_id))
				$ttlJobitSkids:=$ttlJobitSkids+$aSkidCount{$i}
				$text:=$text+$aBin_id{$i}+$delim+String:C10($aSkidCount{$i})+"\r"
			End for 
			$text:=$text+"TOTALS"+$delim+String:C10($ttlJobitSkids)+"\r"
			...
			$text:=$text+"FROM_AREA"+$delim+"NUM_SKIDS"+"\r"
			$ttlJobitSkids:=0
			For ($i; 1; Size of array:C274($aBin_id))
				$ttlJobitSkids:=$ttlJobitSkids+$aSkidCount{$i}
				$text:=$text+$aBin_id{$i}+$delim+String:C10($aSkidCount{$i})+"\r"
			...
			$text:=$text+"SKID"+$delim+"FROM_BIN"+$delim+"CASES"+$delim+"QUANTITY"+$delim+"WHEN_SCANNED"+"\r"
			$ttlJobitCases:=0
			$ttlJobitSkids:=0
			$ttlJobitQty:=0
			For ($i; 1; Size of array:C274($aSkid))
				...
				$text:=$text+"#"+$aSkid{$i}+$delim+$aFromBin_id{$i}+$delim+String:C10($aNumCases{$i})+$delim+String:C10($aQty{$i})+$delim+$aTransAt{$i}+"\r"
			End for 
			$text:=$text+"\rTOTALS\t"+$delim+String:C10($ttlJobitSkids)+$delim+String:C10($ttlJobitCases)+$delim+String:C10($ttlJobitQty)+"\r"
			...
			If (WMS_SQL_Login)  //WMS_API_4D_DoLogin 
				...
				$text:=$text+"BOL"+$delim+"NUM_SKIDS"+"\r"
				$ttlJobitSkids:=0
				For ($i; 1; Size of array:C274($aBin_id))
					$ttlJobitSkids:=$ttlJobitSkids+$aSkidCount{$i}
					$text:=$text+$aBin_id{$i}+$delim+String:C10($aSkidCount{$i})+"\r"
				...
				Begin SQL
					...
					into :$aBin_id, :$aSkidCount
				End SQL
				$text:=$text+"AREA"+$delim+"NUM_SKIDS"+"\r"
				$ttlJobitSkids:=0
				For ($i; 1; Size of array:C274($aBin_id))
					$ttlJobitSkids:=$ttlJobitSkids+$aSkidCount{$i}
					$text:=$text+$aBin_id{$i}+$delim+String:C10($aSkidCount{$i})+"\r"
