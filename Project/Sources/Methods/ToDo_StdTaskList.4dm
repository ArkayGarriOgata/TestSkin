...
Case of 
	: ($1="pick")
		...
		If ($itemPosition>0)
			...
			GOTO RECORD:C242([To_Do_Tasks_Sets:99]; $refNum)
			$2->:=[To_Do_Tasks_Sets:99]Category:2
			$3->:=[To_Do_Tasks_Sets:99]Task:3
		Else 
			$2->:=""
			$3->:=""
		End if 
	...
	: ($1="init")
		...
		If (Records in selection:C76([To_Do_Tasks_Sets:99])>0)
			SELECTION TO ARRAY:C260([To_Do_Tasks_Sets:99]; $aRecNo; [To_Do_Tasks_Sets:99]Category:2; $aCategory; [To_Do_Tasks_Sets:99]Task:3; $aReason)
